        ...
        <template #action="{ record }">
          <a-space size="mini">
            <a-tooltip :content="$t('system.member.action.view')"
              ><a-button
                :aria-label="$t('system.member.action.view')"
                size="small"
                type="text"
                @click="detailDrawerRef?.onView(record.id)"
                ><template #icon><icon-eye /></template></a-button
            ></a-tooltip>
            <a-tooltip v-if="canUpdateMember" :content="$t('system.member.action.edit')"
              ><a-button
                :aria-label="$t('system.member.action.edit')"
                size="small"
                type="text"
                @click="formModalRef?.onEdit(record.id)"
                ><template #icon><icon-edit /></template></a-button
            ></a-tooltip>
            <a-tooltip v-if="canResetMemberPassword" :content="$t('system.member.action.resetPassword')"
              ><a-button
                :aria-label="$t('system.member.action.resetPassword')"
                size="small"
                type="text"
                @click="passwordModalRef?.onEdit(record)"
                ><template #icon><icon-lock /></template></a-button
            ></a-tooltip>
            <a-tooltip v-if="canInvalidateMemberSessions" :content="$t('system.member.action.invalidateSessions')"
              ><a-button
                :aria-label="$t('system.member.action.invalidateSessions')"
                size="small"
                type="text"
                status="warning"
                @click="handleInvalidateSessions(record)"
...
<script lang="ts" setup>
  ...
  const canUpdateMember = computed(() => hasPermission('system.member.update'));
  const canResetMemberPassword = computed(() => hasPermission('system.member.reset_password'));
  const canInvalidateMemberSessions = computed(() => hasPermission('system.member.invalidate_sessions'));
