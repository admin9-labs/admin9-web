        ...
        <template #append>
          <a-tooltip v-if="canCreateRole" :content="createRoleTooltip">
            <a-button
              ...
              :disabled="!permissionCatalogReady || permissionCatalogLoading"
              @click="handleCreate"
            >
          ...
          <a-space>
            <a-tooltip v-if="canUpdateRole" :content="editRoleTooltip(record)" mini>
              <a-button
                type="text"
                size="small"
                :disabled="isReservedRole(record) || !permissionCatalogReady || permissionCatalogLoading"
...
<script lang="ts" setup>
  ...
  const createRoleTooltip = computed(() => {
    if (!canViewPermissionCatalog.value) return t('system.role.permissionCatalogRequired');
    if (!permissionCatalogReady.value) return t('system.role.permissionCatalogUnavailable');
  ...
  const handleCreate = () => {
    if (!permissionCatalogReady.value || permissionCatalogLoading.value) return;
    editDrawerRef.value?.onCreate(permissions.value);
  };

  const handleEdit = (record: RoleRecord) => {
    if (!isReservedRole(record) && permissionCatalogReady.value && !permissionCatalogLoading.value) {
      editDrawerRef.value?.onEdit(record, permissions.value);
    }
  };
