...
<script lang="ts" setup>
  ...
  const handleUserSaved = async (result: UserSaveResult) => {
    if (result.userId === undefined) pagination.current = 1;
    if (result.userId === userStore.id && result.authenticationInvalidated) {
      if (userStore.logoutSessionGeneration(result.requestSession.generation)) {
        Message.success(t('system.user.sessionInvalidated'));
        await router.replace({ name: 'login' });
      }
      return;
    }
    if (result.userId === userStore.id) await refreshCurrentIdentity();
