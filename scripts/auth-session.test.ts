...
test('a late 401 for an old token replays with the current token', () => {
  const request = { status: 401, retried: false, path: '/api/admin/auth/me', generation: 'session-1', token: 'old' };
...
test('the matching session refreshes once while retried requests fail', () => {
  ...
  assert.equal(
    sessionRetryDecision({ status: 401, retried: false, path: '/api/admin/users?current=1', ...current }, current),
    'refresh'
  );
  assert.equal(sessionRetryDecision({ status: 401, retried: true, path: '/api/admin/users', ...current }, current), 'fail');
});
...
test('login, refresh, and logout never recurse while me and password can recover', () => {
  ...
  assert.equal(decision('/api/admin/auth/login'), 'fail');
  assert.equal(decision('/api/admin/auth/refresh'), 'fail');
  assert.equal(decision('/api/admin/auth/logout'), 'fail');
  assert.equal(decision('/api/admin/auth/me'), 'refresh');
  assert.equal(decision('/api/admin/auth/password'), 'refresh');
});
